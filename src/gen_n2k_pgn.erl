...
write_decode_set1_info(Fd, PGN, Info) ->
    ...
    if
        ...
        true ->
            ...
            io:format(
                Fd,
                "decode_~s_set1(N,__REST1,Acc,__DATA) "
            ...
            io:format(Fd, "  end;\n", []),
            if
                Repeat2 =:= undefined, Fixed1Fs =:= [] ->
                    io:format(
                        Fd,
                        "decode_~s_set1(_,_,Acc,_) ->\n"
                        "    {~s,Acc}.\n\n",
                        [ID, ID]
                    );
                Repeat2 =:= undefined ->
                    ...
                    io:format(
                        Fd,
                        "decode_~s_set1(_,__REST2,Acc,__DATA) ->\n"
                        "    case __REST2 of\n"
                        "       <<~s,_/bitstring>> ~s ->\n"
                        "         ~s {~s,Acc++[~s]};\n"
                        "       <<>> ->\n"
                ...
                true ->
                    ...
                    io:format(
                        Fd,
                        "decode_~s_set1(_,__REST,Acc,__DATA) ->\n"
...
field_matches([], CurBit, _N, ByteAcc, MatchesAcc, FVarsAcc) ->
    % fill with don't care
    if
        CurBit > 0 andalso CurBit < 8 ->
            Var = "_",
            VSize = 8 - CurBit,
...
field_matches0(F, T, Size, CurBit, N, Vars, ByteAcc, MatchesAcc, FVarsAcc) ->
    ...
    if
        CurBit == 8 ->
            ...
            field_matches0(F, T, Size, 0, N, Vars, [], [ByteAcc | MatchesAcc], FVarsAcc);
        Size == 0 ->
            %% we're done with this field
            field_matches(T, CurBit, N, ByteAcc, MatchesAcc, [{F, Vars} | FVarsAcc]);
        true ->
            Var = var(N),
            VSize =
                % we fit in this byte
    ...
    case can_replace_bytes(ByteMatches, Vars) of
        {true, ByteMatches0} ->
            replace_bytes(
                ...
                Fs,
                AllFs,
                [{matched, format_field(F, true, AllFs)} | MatchesAcc],
                FVarsAcc
            );
        false ->
...
skip_until_var({Var, _}, [{Var, _} | T], ByteMatch, ByteMatches, Fs, AllFs, MatchesAcc, FVarsAcc) ->
    case T of
        [] ->
            ...
            replace_bytes(
                ...
                Fs,
                AllFs,
                lists:append(ByteMatch, MatchesAcc),
                FVarsAcc
            );
        _ ->
...
skip_until_var(Var, [_ | T], ByteMatch, ByteMatches, Fs, AllFs, MatchesAcc, FVarsAcc) ->
    skip_until_var(Var, T, ByteMatch, ByteMatches, Fs, AllFs, MatchesAcc, FVarsAcc);
skip_until_var(Var, [], ByteMatch0, [ByteMatch | ByteMatches], Fs, AllFs, MatchesAcc, FVarsAcc) ->
    MatchesAcc1 = lists:append(ByteMatch0, MatchesAcc),
    skip_until_var(Var, ByteMatch, ByteMatch, ByteMatches, Fs, AllFs, MatchesAcc1, FVarsAcc).

skip_until_field(VarAndSz, [{_, Vars} = F | Fs], ByteMatches, AllFs, MatchesAcc, FVarsAcc) ->
    case member_and_rest(VarAndSz, Vars) of
        {true, []} ->
            replace_bytes(ByteMatches, Fs, AllFs, MatchesAcc, [F | FVarsAcc]);
        {true, RestVars} ->
            [ByteMatch | ByteMatches1] = ByteMatches,
            LastVar = lists:last(RestVars),
            skip_until_var(
                LastVar, ByteMatch, ByteMatch, ByteMatches1, Fs, AllFs, MatchesAcc, [F | FVarsAcc]
            );
        false ->
            skip_until_field(VarAndSz, Fs, ByteMatches, AllFs, MatchesAcc, [F | FVarsAcc])
    end.
...
replace_single_var([{F, [{Var, _} | T]} = FVar | Fs], Matches, AllFs, FVarsAcc) ->
        ...
        undefined ->
            ...
            case ?getval(type, F) of
                ...
                ->
                    ...
                    if
                        is_integer(RangeMax), RangeMax < MaxVal ->
                            [
...
mk_type_info_fs(PGNId, [[{order, _}, {id, IdStr} | T] | Fs], Tab) ->
    ...
    case get_type(T) of
        ...
        {int, Resolution, Decimals, Unit} ->
            ets:insert(Tab, {{PGNId, Id}, {int, Resolution, Decimals, Unit}});
        {float, Unit} ->
...
get_type(Info) ->
    case {?getval(enum, Info), ?getval(enum2, Info), ?getval(bit, Info)} of
        ...
        {undefined, Enum2Name, undefined} ->
            {enum2, list_to_atom(?getval(enum2_field, Info)), Enum2Name};
...
write_type_info0([{{PGNId, Id}, {enum2, FieldId, EnumName}} | T], Fd) ->
    ...
    io:format(
        Fd,
        "type_info(~p,~p) -> {enums, ~p, enums2_~s()};\n",
